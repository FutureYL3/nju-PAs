...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 0010111", auipc  , U , R(rd) = s->pc + imm);	
	...
	INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, do { if (imm == 0x305) {if (rd != 0) R(rd) = cpu.mtvec; cpu.mtvec = src1;} else if (imm == 0x300) {if (rd != 0) R(rd) = cpu.mstatus; cpu.mstatus = src1;} else if (imm == 0x341) {if (rd != 0) R(rd) = cpu.mepc; cpu.mepc = src1;} else if (imm == 0x342) {if (rd != 0) R(rd) = cpu.mcause; cpu.mcause = src1;} else if (imm == 0x180) {if (rd != 0) R(rd) = cpu.satp; cpu.satp = src1;} else panic("unknown command at pc = " FMT_WORD "\n", s->pc); } while (0));
