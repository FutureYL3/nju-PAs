...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 0010111", auipc  , U , R(rd) = s->pc + imm);	
	...
	INSTPAT("??????? ????? ????? 000 ????? 1100111", jalr   , I , s->dnpc = (imm + src1) & ~1; R(rd) = s->snpc; if (rd == 1 && CONFIG_FTRACE)  ftrace_call_write(s->pc, s->dnpc); else if (CONFIG_FTRACE && rd == 0 && src1 == 1 && imm == 0)  ftrace_ret_write(s->pc, s->dnpc));
